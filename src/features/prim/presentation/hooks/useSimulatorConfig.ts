 ...
 * Handles nodes, edges, root node state, and graph generation/loading.
 */
export function useSimulatorConfig({
  ...
  const [rootNode, setRootNode] = useState<number>(initialRootNode);
  ...
  const loadCustomGraph = useCallback(() => {
    try {
      ...
      // Converte para os tipos específicos do Prim
      const primNodes: Node[] = customNodes.map(n => ({
        id: n.id,
        label: n.label,
        x: n.x,
        y: n.y,
      ...
      const primEdges: Edge[] = customEdges.map(e => ({
        from: e.from,
        to: e.to,
